...
class MessagesService:
    ...
    async def get_conversations(db: AsyncSession, user_id: UUID):
        ...
        try:
            query = select(Conversation).join(ConversationParticipant).where(ConversationParticipant.user_id == user_id).options(
            ...
            for conv in conversations:
                ...
                other_person: Optional[UserProfile] = next((p.user for p in conv.participants if p.user_id != user_id), None)


                full_name = getattr(other_person, "full_name", "Deleted User")
                profile_image_url = getattr(other_person, "profile_image_url", "Invalid URL")
                
                formatted_conversations.append({
                    "conversation_id": conv.conversation_id,
                    "other_user_name": full_name,
                    "other_user_avatar": profile_image_url,
                    "latest_message": conv.latest_message.content,
