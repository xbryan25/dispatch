...
class TestCheckUsername:
    ...
    async def test_check_username_taken(self, client):
        response = await client.get("/api/auth/check-username", params={"username": "newUsername"})
        assert response.status_code == 200
...
class TestUpdateUserDetails:
    ...
    async def test_update_user_details(self, authenticated_client):
        response = await authenticated_client.patch("/api/auth/user-details", json={
    ...
    async def test_update_user_details_unauthenticated(self, client):
        response = await client.patch("/api/auth/user-details", json={
            "username": "newUsername"
        })
        assert response.status_code == 401  
    ...
    async def test_update_user_details_with_invalid_cookie(self, client):
        ...
        response = await client.patch("/api/auth/user-details", headers=headers, json={
            "username": "newUsername"
        })
        assert response.status_code == 401            
...
class TestGetProfileImageUploadUrl:
    ...
    async def test_get_upload_url_invalid_file_type(self, authenticated_client, mock_s3):
        response = await authenticated_client.get(
            "/api/auth/profile-image-upload-url",
    ...
    async def test_update_user_details_with_invalid_cookie(self, client):
        ...
        response = await client.get("/api/auth/profile-image-upload-url", headers=headers, params={"filename": "photo.jpg", "file_type": "image/jpeg"})
        assert response.status_code == 401   
...
class TestUpdateProfileImage:
    ...
    async def test_update_profile_image_invalid_url(self, authenticated_client, mock_s3):
        response = await authenticated_client.patch(
            "/api/auth/profile-image",
    ...
    async def test_update_profile_image__not_in_s3(self, authenticated_client, mock_s3):
